<template>
    <button :class="{'w-button': true, [type]: !!type, primary, disabled, danger}" ref="button" @click="clickAnimation">
      <slot></slot>
    </button>
</template>
...
function clickAnimation(e) {
  ...
  const theme = props.danger ? themeColorVar.danger : (themeColorVar[props.type] || themeColorVar.primary)
  btnNode.animate({
    boxShadow: [
      `0 0 0 0px ${theme}`, 
      `0 0 0 3px ${theme}`,
